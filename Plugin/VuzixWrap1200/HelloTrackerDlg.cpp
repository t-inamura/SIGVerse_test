	...
	, m_srv(NULL)
{
	...
	if(__argc <= 1)
		MessageBox( "Cannot get server hostname and port" ,"Error", MB_OK);

	TCHAR spd[8];
	GetPrivateProfileString(_T("SETTING"),_T("SEND_SPEED"),'\0', spd, 256, _T("./SIGHMD.ini"));

	if(spd[0] == '\0') {
		m_speed = 1;
	}
	else {
		m_speed = atoi((char*)spd);
	}

	std::string sname = "SIGHMD";
	TCHAR name[64];
	GetPrivateProfileString(_T("SETTING"),_T("SERVICE_NAME"),'\0', name, 256, _T("./SIGHMD.ini"));
	if(spd[0] != '\0') {
		sname = name;
	}
	...
	unsigned short port = (unsigned short)(atoi(__argv[2]));
	m_srv->connect(__argv[1],port);
	m_srv->connectToViewer();
	m_srv->setAutoExitProc(true);
}

CHelloTrackerDlg::~CHelloTrackerDlg()
{
	m_srv->disconnect();
	delete m_srv;
	m_srv = NULL;
...
void CHelloTrackerDlg::OnTimer(UINT nIDEvent)
{
	if (nIDEvent == POLL_TIMER) {
		...
		LONG yaw = 0,pitch = 0,roll = 0, xtrn,ytrn,ztrn,myaw = 0;
        ...
        IWRGetMagYaw(&myaw);
		UpdateData(TRUE);
		double dyaw = -(double)yaw/32400.0*M_PI;
		double dpitch = (double)pitch/32400.0*M_PI;
		double droll = -(double)roll/32400.0*M_PI;
		m_csYaw.Format(_T("%1.2f"), dyaw);
		m_csPitch.Format(_T("%1.2f"), dpitch);
		m_csRoll.Format(_T("%1.2f"), droll);
		...
		if(m_sigcount <= 0) {		
			...
			names = m_srv->getAllConnectedEntitiesName();

			int entSize = names.size();
			for(int i = 0; i < entSize; i++) {
				std::string msg = "HMD_DATA ";
				msg += DoubleToString(dyaw);
				msg += DoubleToString(dpitch);
				msg += DoubleToString(droll);
				m_srv->sendMsgToCtr(names[i], msg);
			}
			m_sigcount = m_speed;
		}
		else
